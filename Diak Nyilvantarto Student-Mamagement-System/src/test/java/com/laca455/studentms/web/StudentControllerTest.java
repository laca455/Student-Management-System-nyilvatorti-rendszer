package com.laca455.studentms.web;
...
import com.laca455.studentms.controller.StudentController;
import com.laca455.studentms.service.StudentService;
...
class StudentControllerTest {
	...
	private static final LocalDate DOB_2 = LocalDate.of(2000, 01, 01);
	private static final String EMAIL_2 = "teszt.com";
	...
	void shouldFindStudentByEmail() throws Exception {
		...
		String url = "/api/v1/student-ms/email/laca4555@gmail.com";
	...
	void shouldGetStudentsBetweenTwoDOB() throws Exception {
		LocalDate date1 = LocalDate.of(1999, 11, 20);
		LocalDate date2 = LocalDate.of(2000, 1, 01);
		...
		String url = "/api/v1/student-ms/date-of-birth?dob1=1999-11-20&dob2=2000-01-01";
		MvcResult mvcResult = mockMvc.perform(get(url)).andExpect(status().isOk()).andReturn();
		String actualResponse = mvcResult.getResponse().getContentAsString();
	...
	void shouldUpdateStudentById() throws Exception {
		...
		mockMvc.perform(put(url).contentType(MediaType.APPLICATION_JSON).characterEncoding("utf-8").content(json)
				...
				.andExpect(jsonPath("$.firstName", Matchers.equalTo("Papp")));
	}
	...
	void shouldDeleteStudentByEmail() throws Exception {
		...
		String url = "/api/v1/student-ms/delete-with-email//laca4555n@gmail.com";
		mockMvc.perform(delete(url)).andExpect(status().isAccepted());
